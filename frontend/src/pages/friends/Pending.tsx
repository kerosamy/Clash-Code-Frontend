...
export default function Pending() {
  ...
  useEffect(() => {
    const fetchPending = async () => {
      ...
      try {
        const response = await getPendingFriendRequests(currentPage, pageSize);
  ...
  const handleAccept = async (username: string) => {
    try {
      ...
      setPending(prev => prev.filter(u => u.username !== username));
    } catch (error) {
  ...
  if (loading) {
    return <LogoLoader loadingMessage="Loading Pending Requests" />;
  }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <h1 className="text-3xl font-bold text-orange font-anta">Pending Requests</h1>
      ...
      <Board
        ...
        renderRow={(user) => (
          <UserRow
            key={user.username}
            order={pending.indexOf(user) + 1 + currentPage * pageSize}
